...
activity_file_list_url = reverse('activity_file_list')
activity_file_detail_url = lambda id: reverse('activity_file_detail', args=[id])
...
class ActivityFileTest(OnedriveTestCase):
    ...
    # 完整的上传、删除过程
    def test_upload_activity_file_whole_process(self):
        client = Client()
        ...
        self.assertEqual(self.activity.file.count(), 0)

    # 没有 access token

    # 没有 refresh token 和 access token
