...
class LogoutTest(TestCase):
    def test_log_out(self):
        ...
        r = r.client.post('/users/logout/')
        self.assertEqual(r.status_code, 204)
        self.assertEqual(r.wsgi_request.user.is_authenticated, False)


class ResetPasswordByEmailTest(TestCase):
    ...
    def test_reset_password_by_email_whole_process(self):
        c = Client()
        ...
        c2 = Client()
        r = c2.post('/users/forgetpassword/', {"email": self.email})
        self.assertEqual(r.status_code, 202)

        token = pop_token_from_virtual_mailbox(self)
        r = c2.post('/users/resetpassword/', {
            "token": token
        })
    ...
    def test_forget_email_with_invalid_field(self):
        ...
        r = c.post('/users/forgetpassword/', {"email": "12312313"})
        self.assertEqual(r.status_code, 400)
        r = c.post('/users/forgetpassword/', {"email": "Admin"})
        self.assertEqual(r.status_code, 400)
        r = c.post('/users/forgetpassword/', {"email": "example@example.com"})
        self.assertEqual(r.status_code, 400)
        r = c.post('/users/forgetpassword/', {"email": self.email})
        self.assertEqual(r.status_code, 202)
    ...
    def test_reset_password_by_email_frequently(self):
        """
        ...
        for tc in testcases:
            ...
            r = Client().post('/users/forgetpassword/', {"email": self.user.username})
            self.assertEqual(r.status_code, 202)
            r = Client().post('/users/forgetpassword/', {"email": self.user.username})
            self.assertEqual(r.status_code, 403)
...
class ResetPasswordByPasswordTest(TestCase):
    email = "admin@example.com"
    password = "adminadmin"
    ...
    def setUp(self):
        ...
        self.password_url = '/users/' + str(self.user.id) + '/password/'

    def test_reset_password_by_password(self):
        c1 = Client()
    ...
    def test_reset_password_by_password_with_invalid_field(self):
        tester_signup("another@example.com", "anotheruser", "another", "20201231")
        anotheruser = User.objects.filter(username="another@example.com").first()
        # 没上线
        c = Client()
        ...
        self.assertEqual(r.status_code, 200)
        r = c.post(self.password_url, {"old_password": self.password, "new_password": "ADMINADMIN"}) # 访问别人的修改密码 API
        self.assertEqual(r.status_code, 403)
        # 少字段
        r = tester_login(self.email, self.password, c)
